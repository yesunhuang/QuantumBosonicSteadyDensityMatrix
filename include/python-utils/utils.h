    ...
    bool CheckArgsCount(unsigned n) {
        ...
        if (now < n) {
            Py_RaiseError(PyExc_TypeError,
                "Too few arguments required, given %u, but require %u. Object: %R",
                now, n, this->args);
            return false;
        } else if (now > n) {
            Py_RaiseError(PyExc_TypeError,
                "Too many arguments required, given %u, but require %u. Object: %R",
                now, n, this->args);
            return false;
        }
        return true;
...
public:
    explicit PyListReader(PyObject* o): obj(o) {
        if (!PyList_Check(o)) {
            ...
            Py_RaiseError(PyExc_TypeError, "Argument not a tuple or list. Object: %R", o);
        }
    }
    /**
    ...
    PyObject* GetItem(unsigned index) {
        if (index >= static_cast<unsigned>(this->size)) {
            Py_RaiseError(PyExc_IndexError, "Out of index(%u of %u). Object: %R",
                index, GetSize(), this->obj);
            return nullptr;
        }
        switch (this->type) {
        case ListType::List:
 ...
 */
template<typename T>
inline T PyObjectToNum(PyObject* o) {
...
template<>
inline ayaji::Complex PyObjectToNum(PyObject* o) {
    if (PyComplex_CheckExact(o)) {
        return ayaji::Complex{PyComplex_RealAsDouble(o), PyComplex_ImagAsDouble(o)};
    } else if (PyFloat_CheckExact(o)) {
...
template<typename T>
inline std::vector<T> PyListToNum(PyObject* o) {
    PyListReader reader(o);
    std::vector<int> ret;
 ...
 */
template<typename T>
inline std::vector<T> PyListTo(PyObject* o) {
    static_assert("Not implemented yet!");
}
template<>
inline std::vector<RawTerm> PyListTo(PyObject* o) {
    PyListReader reader(o);
