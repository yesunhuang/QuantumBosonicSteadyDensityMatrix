...
private:
    ...
    inline std::vector<int> getNeighbour(std::vector<int> root,
                                                   int index) {
    ...
    void doRun(std::vector<int> index, int depth, int _i){
        if(depth == matrixSizeArray.size()){
            ...
            do {
                ...
                // 更新原值
                dst[_i]->set(root, src[_i]->get(root) + alpha * (gamma * epDeriver.calMasterEP(root) / epDeriver.calNeighbourEP(root, nextNeighbourIndex) * src[_i]->get(nextNeighbourIndex) - src[_i]->get(root)));
                root = nextNeighbour;
            }while (inbound(root));
        }else{
